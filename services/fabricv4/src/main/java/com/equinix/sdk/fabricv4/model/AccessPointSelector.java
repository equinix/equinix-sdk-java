...
public class AccessPointSelector {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(type, hideAssetInfo, port, linkProtocol, virtualDevice, _interface, network);
  }
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("type", "hideAssetInfo", "port", "linkProtocol", "virtualDevice", "interface", "network"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!AccessPointSelector.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in AccessPointSelector is not found in the empty JSON string", AccessPointSelector.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!AccessPointSelector.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `AccessPointSelector` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
