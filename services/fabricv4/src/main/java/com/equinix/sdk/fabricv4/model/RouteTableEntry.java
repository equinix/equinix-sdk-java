...
public class RouteTableEntry {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(type, protocolType, state, prefix, nextHop, MED, localPreference, asPath, connection, changeLog);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RouteTableEntry.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in RouteTableEntry is not found in the empty JSON string", RouteTableEntry.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RouteTableEntry.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `RouteTableEntry` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
