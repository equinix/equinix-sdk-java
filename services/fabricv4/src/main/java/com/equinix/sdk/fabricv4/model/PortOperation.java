...
public class PortOperation {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(operationalStatus, connectionCount, evplVCCount, fgVCCount, accessVCCount, opStatusChangedAt);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PortOperation.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in PortOperation is not found in the empty JSON string", PortOperation.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PortOperation.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `PortOperation` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
