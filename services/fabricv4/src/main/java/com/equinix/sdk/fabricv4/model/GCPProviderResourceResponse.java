...
public class GCPProviderResourceResponse extends AbstractOpenApiSchema {
            ...
            return (TypeAdapter<T>) new TypeAdapter<GCPProviderResourceResponse>() {
                ...
                public GCPProviderResourceResponse read(JsonReader in) throws IOException {
                    ...
                    try {
                        ...
                        // deserialization failed, continue
                        errorMessages.add(String.format(Locale.ROOT, "Deserialization for GCPCloudRouterResponse failed with `%s`.", e.getMessage()));
                    ...
                    throw new IOException(String.format(Locale.ROOT, "Failed deserialization for GCPProviderResourceResponse: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format(Locale.ROOT, "The JSON string is invalid for GCPProviderResourceResponse with oneOf schemas: GCPCloudRouterResponse, GCPPermission. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }
