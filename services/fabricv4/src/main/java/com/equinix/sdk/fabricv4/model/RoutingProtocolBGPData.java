...
public class RoutingProtocolBGPData {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(type, name, bgpIpv4, bgpIpv6, customerAsn, equinixAsn, bgpAuthKey, asOverrideEnabled, bfd, href, uuid, state, operation, change, changelog, project, connection);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!RoutingProtocolBGPData.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in RoutingProtocolBGPData is not found in the empty JSON string", RoutingProtocolBGPData.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!RoutingProtocolBGPData.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `RoutingProtocolBGPData` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
