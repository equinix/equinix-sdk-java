...
public class StreamAsset {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(href, uuid, type, metricsEnabled, attachmentStatus);
  }
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("href", "uuid", "type", "metricsEnabled", "attachmentStatus"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!StreamAsset.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in StreamAsset is not found in the empty JSON string", StreamAsset.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!StreamAsset.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `StreamAsset` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
