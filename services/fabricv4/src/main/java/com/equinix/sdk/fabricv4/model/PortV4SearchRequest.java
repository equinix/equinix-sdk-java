...
public class PortV4SearchRequest {
  ...
  public PortV4SearchRequest filter(@javax.annotation.Nonnull PortExpression filter) {
    this.filter = filter;
    return this;
  }
  ...
  public void setFilter(@javax.annotation.Nonnull PortExpression filter) {
    this.filter = filter;
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PortV4SearchRequest.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in PortV4SearchRequest is not found in the empty JSON string", PortV4SearchRequest.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PortV4SearchRequest.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `PortV4SearchRequest` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : PortV4SearchRequest.openapiRequiredFields) {
