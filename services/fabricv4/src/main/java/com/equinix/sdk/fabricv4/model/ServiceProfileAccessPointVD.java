...
public class ServiceProfileAccessPointVD {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("type", "uuid", "location", "interfaceUuid"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ServiceProfileAccessPointVD.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in ServiceProfileAccessPointVD is not found in the empty JSON string", ServiceProfileAccessPointVD.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ServiceProfileAccessPointVD.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ServiceProfileAccessPointVD` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if ((jsonObj.get("interfaceUuid") != null && !jsonObj.get("interfaceUuid").isJsonNull()) && !jsonObj.get("interfaceUuid").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `interfaceUuid` to be a primitive type in the JSON string but got `%s`", jsonObj.get("interfaceUuid").toString()));
      }
  }
