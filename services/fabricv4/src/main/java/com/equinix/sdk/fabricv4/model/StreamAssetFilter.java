...
public class StreamAssetFilter extends AbstractOpenApiSchema {
            ...
            return (TypeAdapter<T>) new TypeAdapter<StreamAssetFilter>() {
                ...
                public StreamAssetFilter read(JsonReader in) throws IOException {
                    ...
                    try {
                        ...
                        errorMessages.add(String.format(Locale.ROOT, "Deserialization for StreamAssetSimpleExpression failed with `%s`.", e.getMessage()));
                    ...
                    try {
                        ...
                        // deserialization failed, continue
                        errorMessages.add(String.format(Locale.ROOT, "Deserialization for StreamAssetOrFilter failed with `%s`.", e.getMessage()));
                        log.log(Level.FINER, "Input data does not match schema 'StreamAssetOrFilter'", e);
                    }

                    throw new IOException(String.format(Locale.ROOT, "Failed deserialization for StreamAssetFilter: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format(Locale.ROOT, "The JSON string is invalid for StreamAssetFilter with anyOf schemas: StreamAssetOrFilter, StreamAssetSimpleExpression. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }
