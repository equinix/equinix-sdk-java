...
import com.equinix.sdk.fabricv4.model.VirtualConnectionPriceZSideAccessPoint;
...
public class VirtualConnectionPriceZSide {
  ...
  public VirtualConnectionPriceZSide accessPoint(@javax.annotation.Nullable VirtualConnectionPriceZSideAccessPoint accessPoint) {
  ...
  public void setAccessPoint(@javax.annotation.Nullable VirtualConnectionPriceZSideAccessPoint accessPoint) {
    this.accessPoint = accessPoint;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to VirtualConnectionPriceZSide
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VirtualConnectionPriceZSide.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in VirtualConnectionPriceZSide is not found in the empty JSON string", VirtualConnectionPriceZSide.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VirtualConnectionPriceZSide.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `VirtualConnectionPriceZSide` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      // validate the optional field `accessPoint`
      if (jsonObj.get("accessPoint") != null && !jsonObj.get("accessPoint").isJsonNull()) {
        VirtualConnectionPriceZSideAccessPoint.validateJsonElement(jsonObj.get("accessPoint"));
      }
  }
