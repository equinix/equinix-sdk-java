            ...
            return (TypeAdapter<T>) new TypeAdapter<CloudRouterActionsSearchFilter>() {
                ...
                public CloudRouterActionsSearchFilter read(JsonReader in) throws IOException {
                    ...
                    try {
                        ...
                        errorMessages.add(String.format(Locale.ROOT, "Deserialization for CloudRouterActionsSearchExpression failed with `%s`.", e.getMessage()));
                    ...
                    try {
                        ...
                        // deserialization failed, continue
                        errorMessages.add(String.format(Locale.ROOT, "Deserialization for CloudRouterActionsSearchOrFilter failed with `%s`.", e.getMessage()));
                        log.log(Level.FINER, "Input data does not match schema 'CloudRouterActionsSearchOrFilter'", e);
                    }

                    throw new IOException(String.format(Locale.ROOT, "Failed deserialization for CloudRouterActionsSearchFilter: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
