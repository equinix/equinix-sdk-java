...
public class PortOrderSignature {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("signatory", "delegate"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!PortOrderSignature.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in PortOrderSignature is not found in the empty JSON string", PortOrderSignature.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!PortOrderSignature.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `PortOrderSignature` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
        JsonObject jsonObj = jsonElement.getAsJsonObject();
      if ((jsonObj.get("signatory") != null && !jsonObj.get("signatory").isJsonNull()) && !jsonObj.get("signatory").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `signatory` to be a primitive type in the JSON string but got `%s`", jsonObj.get("signatory").toString()));
      }
