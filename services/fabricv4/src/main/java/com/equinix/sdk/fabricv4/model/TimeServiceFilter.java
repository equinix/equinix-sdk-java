...
public class TimeServiceFilter extends AbstractOpenApiSchema {
            ...
            return (TypeAdapter<T>) new TypeAdapter<TimeServiceFilter>() {
                ...
                public TimeServiceFilter read(JsonReader in) throws IOException {
                    ...
                    try {
                        ...
                        errorMessages.add(String.format(Locale.ROOT, "Deserialization for TimeServiceSimpleExpression failed with `%s`.", e.getMessage()));
                    ...
                    try {
                        ...
                        // deserialization failed, continue
                        errorMessages.add(String.format(Locale.ROOT, "Deserialization for TimeServiceOrFilter failed with `%s`.", e.getMessage()));
                        log.log(Level.FINER, "Input data does not match schema 'TimeServiceOrFilter'", e);
                    }

                    throw new IOException(String.format(Locale.ROOT, "Failed deserialization for TimeServiceFilter: no class matches result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        throw new IOException(String.format(Locale.ROOT, "The JSON string is invalid for TimeServiceFilter with anyOf schemas: TimeServiceOrFilter, TimeServiceSimpleExpression. no class match the result, expected at least 1. Detailed failure message for anyOf schemas: %s. JSON: %s", errorMessages, jsonElement.toString()));
    }
