...
public class TopologyProperties {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("elementId", "dependsOn"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("elementId"));
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!TopologyProperties.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in TopologyProperties is not found in the empty JSON string", TopologyProperties.openapiRequiredFields.toString()));
        }
      }
      ...
      if (!jsonObj.get("elementId").isJsonPrimitive()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `elementId` to be a primitive type in the JSON string but got `%s`", jsonObj.get("elementId").toString()));
      }
      // ensure the optional json data is an array if present
      if (jsonObj.get("dependsOn") != null && !jsonObj.get("dependsOn").isJsonNull() && !jsonObj.get("dependsOn").isJsonArray()) {
        throw new IllegalArgumentException(String.format(Locale.ROOT, "Expected the field `dependsOn` to be an array in the JSON string but got `%s`", jsonObj.get("dependsOn").toString()));
      }
  }
