...
public class StreamAlertRule {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(href, uuid, type, name, description, state, enabled, metricSelector, resourceSelector, detectionMethod, changeLog);
  }
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("href", "uuid", "type", "name", "description", "state", "enabled", "metricSelector", "resourceSelector", "detectionMethod", "changeLog"));
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!StreamAlertRule.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in StreamAlertRule is not found in the empty JSON string", StreamAlertRule.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!StreamAlertRule.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `StreamAlertRule` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
