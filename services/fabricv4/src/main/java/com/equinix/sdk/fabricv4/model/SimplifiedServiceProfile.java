...
public class SimplifiedServiceProfile {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(href, type, name, uuid, description, notifications, tags, visibility, allowedEmails, accessPointTypeConfigs, customFields, marketingInfo, ports, virtualDevices, metros, environments, selfProfile, projectId, lastMileConfig);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SimplifiedServiceProfile.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in SimplifiedServiceProfile is not found in the empty JSON string", SimplifiedServiceProfile.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SimplifiedServiceProfile.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `SimplifiedServiceProfile` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
