...
public class VpicInterface {
  ...
  public VpicInterface uuid(@javax.annotation.Nullable UUID uuid) {
    this.uuid = uuid;
    return this;
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to VpicInterface
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!VpicInterface.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The required field(s) %s in VpicInterface is not found in the empty JSON string", VpicInterface.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!VpicInterface.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(Locale.ROOT, "The field `%s` in the JSON string is not defined in the `VpicInterface` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
